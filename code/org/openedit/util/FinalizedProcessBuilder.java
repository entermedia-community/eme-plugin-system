 ...
 * 
 * <p>
 * Starting a new process is just as easy as {@link ProcessBuilder}:
 *
 * <pre>
 * {
 * 	&#064;code
 * 	FinalizedProcess process = new FinalizedProcessBuilder(&quot;myCommand&quot;, &quot;myArg&quot;).start(executorManager);
 * }
 * </pre>
 *
 * <p>
 * Standard output and standard error are always gobbled and logged on separate threads, and are never merged
 * (there is no way to opt into {@code ProcessBuilder#redirectErrorStream}), so a subprocess can never block the
 * caller by filling either pipe. The subprocess is always destroyed when the returned {@link FinalizedProcess} is
 * closed — this class is for running a command to completion, not for starting a long-lived daemon.
 *
 * @author John Leacox
 * @see ProcessBuilder
 */
public class FinalizedProcessBuilder {
	...
	private Consumer<String> outputLineListener;
	private Consumer<String> errorLineListener;
	...
	/**
	 * Sets a callback invoked with each line of standard output as it is read from the
	 * subprocess, in addition to the standard output buffering that is always performed.
	 * May be null (the default) to disable live line callbacks.
	 *
	 * @param inListener the line callback, or null
	 * @return this process builder
	 */
	public FinalizedProcessBuilder onOutputLine(Consumer<String> inListener) {
		this.outputLineListener = inListener;
		return this;
	}

	/**
	 * Sets a callback invoked with each line of standard error as it is read from the
	 * subprocess, in addition to the standard error buffering that is always performed.
	 * May be null (the default) to disable live line callbacks.
	 *
	 * @param inListener the line callback, or null
	 * @return this process builder
	 */
	public FinalizedProcessBuilder onErrorLine(Consumer<String> inListener) {
		this.errorLineListener = inListener;
		return this;
	}
	...
	public FinalizedProcess start(ExecutorManager inManager) throws IOException
	{
		...
		StreamGobbler inputGobbler = new StreamGobbler(process.getInputStream(), true, outputLineListener);
		inManager.execute(inputGobbler);
		gobblers.add(inputGobbler);

		StreamGobbler errorGobbler = new StreamGobbler(process.getErrorStream(), true, errorLineListener);
		errorGobbler.setErrorStream(true);
		inManager.execute(errorGobbler);
		gobblers.add(errorGobbler);

		return new FinalizedProcess(process, gobblers);
	}
